...
def main(settings):
    num_folds = len(settings['models'][0]['configs'])
    df_list = []
    ...
    for fold in settings['folds']:
        ...
        for model_settings in settings['models']:
            # with open('configs/' + model_settings['configs'][fold]['config_name'] + '.yaml', "r") as yamlfile:
            ...
            if isinstance(config.TEST.CHECKPOINT, int):
                ...
                if 'best' in config.TEST.CHECKPOINT:
                    checkpoint_path = os.path.join(config.DATA.OUTPUT_DIR, model_settings['configs'][fold]['exp_name'], 'ckpt', 'checkpoint_best.pth')
                elif '/' in config.TEST.CHECKPOINT:
                    checkpoint_path = config.TEST.CHECKPOINT
                else:
                    if (config.TEST.CHECKPOINT).endswith('.pth'):
                        checkpoint_path = os.path.join(config.DATA.OUTPUT_DIR, model_settings['configs'][fold]['exp_name'], 'ckpt', config.TEST.CHECKPOINT)
                    else:
                        checkpoint_path = ''
            else:
                checkpoint_path = ''
        ...
        if settings['save_results']:
            fig.savefig(os.path.join(save_dir, f'ROC_PR_Curves_fold_{fold}.jpg'), dpi=200)
            cur_df.round(6).to_csv(os.path.join(save_dir, f'metrics_table_fold{fold}.csv'), index=False)
        else:
            plt.show()
